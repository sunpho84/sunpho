...
namespace bissa
{
  ...
  void ILDG_File_read_ildg_data_all(void *data,ILDG_File &file,ILDG_header &header)
  {
    ...
    //reorder
    int *order=bissa_malloc("order",loc_vol,int);
    BISSA_LOC_VOL_LOOP(idest)
    {
      int isour=0;
      for(int mu=0;mu<4;mu++)
	{
    ...
    char *buf=bissa_malloc("buf",nbytes_per_rank_exp,char);
  ...
  void ILDG_File_read_checksum(checksum check_read,ILDG_File &file)
  {
	...
	char *mess=bissa_malloc("mess",nbytes+1,char);
  ...
  THREADABLE_FUNCTION_3ARG(remap_to_write_ildg_data, char*,buf, char*,data, int,nbytes_per_site)
  {
    ...
    BISSA_PARALLEL_LOOP(isour,0,loc_vol)
    {
      int idest=0;
      for(int mu=0;mu<4;mu++)
	{
  ...
  void ILDG_File_write_ildg_data_all(ILDG_File &file,void *data,ILDG_Offset nbytes_per_site,const char *type)
  {
    ...
    char *buf=bissa_malloc("buf",nbytes_per_rank,char);
  ...
  void ILDG_File_write_record(ILDG_File &file,const char *type,const char *ext_buf,uint64_t ext_len)
  {
    ...
    char *buf_out=bissa_malloc("buf",len,char);
