...
double fun_fit_F(double A,double B,double C,double D,double ml,double a)
{
  ...
  return A*(1 + B*ml + C*a1*a1) - include_ml_term*3/(2*M_PI*M_PI)*log(ml)*ml +D/a1;
  
  /*
  double co=(db0/(4*M_PI*f0*f0)).data[plot_iboot];  
  return A*(1 + (B-co*log(ml))*ml + C*a1*a1 +D*a1*a1*a1*a1);
  */
}
...
void plot_funz_ml(const char *out_path,const char *title,const char *xlab,const char *ylab,bvec &X,bvec &Y,bvec &par,double X_phys,double (*fun)(double,double,double,double,double,double),boot &chiral_extrap_cont)
{
  ...
  if(fun!=NULL)
      ...
      for(int ib=0;ib<nbeta;ib++)
	{
      ...
      for(int ipoint=0;ipoint<npoint;ipoint++)
	for(int iboot=plot_iboot=0;iboot<nboot+1;plot_iboot=iboot++)
	  Y_pol.data[ipoint]=fun(par[0][iboot],par[1][iboot],par[2][iboot],par[3][iboot],X_pol[ipoint],1.e-10);
      //out.set(1,"magenta");
      //out.polygon(X_pol,Y_pol);
      //out.new_set();
      out.set(1,"magenta");
...
int main(int narg,char **arg)
{
  ...
  for(int iens=0;iens<nens;iens++)
    {
      ...
      F[iens]=boot(nboot,njack);
      F[iens].load(combine("../%s/%s",path,meson_name).c_str());
    }
  ...
  for(int iboot=0;iboot<nboot+1;iboot++)
    {
      F_chir_cont.data[iboot]=fun_fit_F(A[iboot],B[iboot],C[iboot],D[iboot]*DIV_REM,ml_phys[iboot],1.e-10);
      for(int ib=0;ib<nbeta;ib++)
	{
	  int r=ref_ml_beta[ib];
	  F_chir[ib].data[iboot]=fun_fit_F(A[iboot],B[iboot],C[iboot],D[iboot],ml_phys[iboot],lat[ib][iboot]);
	  if(r!=-1)
	    F_estr_ml.data[ib].data[iboot]=F[r][iboot]*fun_fit_F(A[nboot],B[nboot],C[nboot],D[iboot]*DIV_REM,ml_phys[nboot],1.e-10)/fun_fit_F(A[nboot],B[nboot],C[nboot],D[iboot]*DIV_REM,ml[r][nboot],1.e-10);
	}
    }
  ...
  boot cchir=db0*f0*f0/4;
  cout<<"(chir cond)^3 = [ "<<smart_print(pow(cchir,1.0/3)*1000)<<" MeV ]^3"<<endl;
  
  cout<<"Final result: "<<smart_print(F_chir_cont/cchir)<<endl;
