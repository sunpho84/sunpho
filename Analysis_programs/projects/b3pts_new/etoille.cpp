...
jvec load_2pts_VKVK(int sme,int im1,int im2)
{
  jvec a=jvec_load(combine("../DATA/2pts_30_%02d",sme).c_str(),T,njacks,icombo_2pts(i2_VKVK,im1,4,im2,0));
  jvec b=jvec_load(combine("../DATA/2pts_30_%02d",sme).c_str(),T,njacks,icombo_2pts(i2_VKVK,im1,4,im2,1));
...
int main(int narg,char **arg)
{
  ...
  for(int isp=0;isp<2;isp++)
    {
      //load 2pts for D
      jvec Detoille_corr_SL=load_2pts_VKVK(0,isp,ic_fix);
      jvec Detoille_corr_SS=load_2pts_VKVK(30,isp,ic_fix);
      ...
      two_pts_SL_fit(MD,ZDL,ZDS,Detoille_corr_SL,Detoille_corr_SS,
		     ...
		     combine("plots/D%c_etoille_effmass_SL.xmg",tag_isp[isp]).c_str(),
		     combine("plots/D%c_etoille_effmass_SS.xmg",tag_isp[isp]).c_str());
	  ...
	  cout<<"ZH["<<ih<<"]sm: "<<smart_print(ZHS)<<endl;
          ...
          //load 3pts
          jvec HD_AK=load_3pts(iAKVK,isp,4,ic_fix,4,ih);
          stamp(combine("%s/B%d%c_Ax_D%c",arg[1],ih,tag_isp[isp],tag_isp[isp]),HD_AK);
	  HD_AK.print_to_file(combine("/tmp/HD%c_%d_AK.xmg",tag_isp[isp],ih).c_str());
	  ...
	  ofstream cfit(combine("plots/HD%c_%d_AK_mel.xmg",tag_isp[isp],ih).c_str());
	  ...
	  numerical_derivative(HD_AK_mel_corr_nu).print_rel_err_to_file
	    (combine("plots/HD%c_%d_AK_der_nu.xmg",tag_isp[isp],ih).c_str());
	  ...
	  jvec HD_BK=load_3pts(iBKVK,isp,4,ic_fix,4,ih);
          stamp(combine("%s/B%d%c_Bx_D%c",arg[1],ih,tag_isp[isp],tag_isp[isp]),HD_BK);
	  /*
	    jvec HD_BK_mel_corr_an=HD_BK/dt_an/(2*sqrt(MD*MH))*Zt_med[ibeta];
	    jvec HD_BK_mel_corr_nu=HD_BK/dt_nu/(2*sqrt(MD*MH))*Zt_med[ibeta];
	    
	    HD_BK_mel_corr_an.print_to_file(combine("plots/HD%c_%d_BK_mel_an.xmg",tag_isp[isp],ih).c_str());
	    HD_BK_mel_corr_nu.print_to_file(combine("plots/HD%c_%d_BK_mel_nu.xmg",tag_isp[isp],ih).c_str());
	    
	    (HD_BK/HD_AK*Zt_med[ibeta]/Za_med[ibeta]).print_to_file(combine("plots/HD%c_%d_BAK_ratio.xmg",tag_isp[isp],ih).c_str());
	  */
	}
    }
