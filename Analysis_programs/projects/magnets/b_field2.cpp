...
void plot(double b)
{
  ...
  for(int y=L-1;y>=0;y--)
	...
	double z1,z2=atan2(a.imag(),a.real());
	if(z2<0) z1=-0.30;
	else z1=0;

	out<<"set arrow "<<iarr++<<" from "<<x+0.5<<","<<y+0.5<<","<<z1<<" to "<<x+0.5<<","<<y+0.5<<","<<z2<<
	  " lc rgb \"red\" lw 1 head front"<<endl;
      }
  out<<"splot 0 w pm3d"<<endl;
  
  system("gnuplot out.gp");
}

int main()
{
  for(double b=2;b<=3.001;b+=0.20)
    plot(b);
  return 0;
}
