...
char basepath[100],plotpath[100],current[100];
double **corr_two_pts_fit,**err_two_pts_fit;
int *tmin_fit;
int *tmax_fit;
int *use_fit;
int *sign,*parity,*r_ity,*ri;
...
void read_input(const char *path)
{
  ...
  r_ity=(int*)malloc(sizeof(int)*nsm*nsm);
  ri=(int*)malloc(sizeof(int)*nsm*nsm);
  expect_string_from_file(fin,"sign_parity_r_ity_ri");
  for(int ism=0;ism<nsm;ism++)
    for(int jsm=0;jsm<nsm;jsm++)
      {
	read_formatted_from_file((char*)(sign+ism*nsm+jsm),fin,"%d","sign");
	read_formatted_from_file((char*)(parity+ism*nsm+jsm),fin,"%d","parity");
	read_formatted_from_file((char*)(r_ity+ism*nsm+jsm),fin,"%d","r_ity");
...
int icombo(int r,int ri)
{
  if(tm_run) return ri+2*(r+2*itheta);
  else return ri+2*itheta;
}

jvec load_2pts(const char *path,int i)
{
  jvec a(T,njacks),b(T,njacks);
  
  int r=0;
  a.load(path,icombo(r,ri[i]));
  if(tm_run==0) return a.simmetrized(parity[i]);
  else
    {
      b.load(path,icombo(!r,ri[i]));
      
      return ((a+r_ity[i]*b)/2).simmetrized(parity[i]);
    }
}
	  ...
	  for(int t=tmin_fit[ism*nsm+jsm];t<=tmax_fit[ism*nsm+jsm];t++)
	    {
	      ...
	      if(flag==3) cout<<ism<<" "<<jsm<<" "<<t<<": [("<<num<<"-"<<teo<<")/"<<err<<"]^2="<<cont<<endl;
	    }
}
  ...
  for(int ist=0;ist<nst;ist++)
	...
	for(int jsm=0;jsm<nsm;jsm++)
	  if(sign[ism*nsm+jsm]!=0) //exclude if sign is 0
	    ...
	    if(ist==0)
	      {
		corr[ist][ism*nsm+jsm]=sign[ism*nsm+jsm]*load_2pts(combine(basepath,sm[ism],sm[jsm]).c_str(),ism*nsm+jsm);
