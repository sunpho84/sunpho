...
{  
  ...
  {
    nper_rank_to_temp=bissa_malloc("nper_rank_to_temp",nranks,int);
    nper_rank_fr_temp=bissa_malloc("nper_rank_fr_temp",nranks,int);
  }
  temp_build_t::~temp_build_t()
  {
    bissa_free(nper_rank_to_temp);
    bissa_free(nper_rank_fr_temp);
    bissa_free(out_buf_cur_per_rank);
    bissa_free(in_buf_cur_per_rank);
  }

  all_to_all_comm_t::~all_to_all_comm_t()
  {
    bissa_free(list_ranks_to);
    bissa_free(list_ranks_fr);
    bissa_free(in_buf_dest);
    bissa_free(out_buf_source);
    bissa_free(nper_rank_fr);
    bissa_free(nper_rank_to);
    bissa_free(out_buf_off_per_rank);
    bissa_free(in_buf_off_per_rank);
  }
  ...
  void all_to_all_comm_t::common_setup_part1(temp_build_t &build)
  {
    ...
    list_ranks_to=bissa_malloc("list_ranks_to",nranks_to,int);
    list_ranks_fr=bissa_malloc("list_ranks_fr",nranks_fr,int);

    //store the true list of ranks to communicate with and the number of elements to send to each rank
    nper_rank_to=bissa_malloc("nper_rank_to",nranks_to,int);
    nper_rank_fr=bissa_malloc("nper_rank_fr",nranks_fr,int);
    ...
    out_buf_off_per_rank=bissa_malloc("out_buf_off",nranks_to,int);
    in_buf_off_per_rank=bissa_malloc(" in_buf_off",nranks_fr,int);
    build.out_buf_cur_per_rank=bissa_malloc("out_buf_cur",nranks_to,int);
    build.in_buf_cur_per_rank=bissa_malloc(" in_buf_cur",nranks_fr,int);
    if(IS_MASTER_THREAD)
      {
	if(nranks_to!=0)
	  {
    ...
    int *buf_expl,int nranks_expl,int *list_ranks_expl,int *buf_expl_off_per_rank,int *nper_rank_expl)
  {
    ...
    buf_note=bissa_malloc("buf_note",nel_note,int);
    if(IS_MASTER_THREAD)
      {
    ...
    //check
    /*
    int *in_buf_dest_check=bissa_malloc("in_buf_dest_check",nel_in,int);
  ...
  void all_to_all_comm_t::setup_knowing_where_to_send(all_to_all_scattering_list_t &sl)
  {
    ...
    out_buf_source=bissa_malloc("out_buf_source",nel_out,int);

    //save where to store and fill where to load each element in the temporary buffer
    int *in_buf_dest_expl=bissa_malloc("in_buf_dest_expl",nel_out,int);
    if(IS_MASTER_THREAD)
      for(int iel_out=0;iel_out<nel_out;iel_out++)
	{
  ...
  void all_to_all_comm_t::setup_knowing_what_to_ask(all_to_all_gathering_list_t &gl)
  {
    ...
    common_setup_part1(build);
    in_buf_dest=bissa_malloc("in_buf_dest",nel_in,int);
    
    //save the explenations to each rank on how to fill outbuffers
    int *out_buf_source_expl=bissa_malloc("out_buf_source_expl",nel_in,int);
    if(IS_MASTER_THREAD)
      for(all_to_all_gathering_list_t::iterator it=gl.begin();it!=gl.end();it++)
	{
    ...
    bissa_free(out_buf_source_expl);
  }
  ...
  void all_to_all_comm_t::communicate(void *out,void *in,int bps,void *ext_out_buf,void *ext_in_buf,int tag)
  {
    ...
    char *out_buf=(ext_out_buf==NULL)?bissa_malloc("out_buf",nel_out*bps,char):(char*)ext_out_buf;
    char *in_buf=(ext_in_buf==NULL)?bissa_malloc("in_buf",nel_in*bps,char):(char*)ext_in_buf;
    
    //copy data on the out-going buffer
    BISSA_PARALLEL_LOOP(iel_out,0,nel_out)
    ...
    BISSA_PARALLEL_LOOP(iel_in,0,nel_in)
      memcpy((char*)out+in_buf_dest[iel_in]*bps,in_buf+iel_in*bps,bps);
    
    set_borders_invalid(out);
    
    if(ext_out_buf==NULL) bissa_free(out_buf);
    if(ext_in_buf==NULL) bissa_free(in_buf);
  }
