...
namespace bissa
{
  //return the pointer to the bissa vect
  bissa_vect* get_vect(void *v)
  {return (bissa_vect*)v-1;}
  
  //return the name of the vector
  char *get_vect_name(void *v)
  {return get_vect(v)->tag;}
  
  //print the content of an bissa vect
  ...
  {vect_content_fprintf(f,(bissa_vect*)vec-1);}
  ...
  {unset_vect_flag(data,BORDERS_VALID);}
  
  //print all bissa vect
  void print_all_vect_content()
  {
    bissa_vect *curr=&(main_vect);
    do
      {  
  ...
  void initialize_main_vect()
  {
	...
	main_arr=(char*)last_vect+sizeof(bissa_vect);
	
	master_printf("Vector memory manager started\n");
      }
  }
  
  //allocate an bissa vector 
  void *internal_bissa_malloc(const char *tag,int nel,int size_per_el,const char *type,const char *file,int line)
  {
	...
	bissa_vect *nv=(bissa_vect*)malloc(size+sizeof(bissa_vect));
	if(nv==NULL)
	...
	nv->flag=0;
	take_last_characters(nv->file,file,BISSA_VECT_STRING_LENGTH);
	take_last_characters(nv->tag,tag,BISSA_VECT_STRING_LENGTH);
	take_last_characters(nv->type,type,BISSA_VECT_STRING_LENGTH);
	...
	int offset=((long long int)(return_malloc_ptr))%BISSA_VECT_ALIGNMENT;
	if(offset!=0)
	  crash("memory alignment problem, vector %s has %d offset",tag,offset);
	
	//if borders are allocated, set appropriate flag
  ...
  void vector_copy(void *a,void *b)
  {
    ...
    if(a!=b)
      {
	//get vector pointers
	bissa_vect *bissa_a=(bissa_vect*)((char*)a-sizeof(bissa_vect));
	bissa_vect *bissa_b=(bissa_vect*)((char*)b-sizeof(bissa_vect));
	
	//get nentries
	int nel_a=bissa_a->nel;
	int nel_b=bissa_b->nel;
	
	//get nentries per site
	int size_per_el_a=bissa_a->size_per_el;
	int size_per_el_b=bissa_b->size_per_el;
	
	//check size agreement
	if(nel_a!=nel_b) crash("while copying, vector %s contains %d and vector %s contains %d",
			       bissa_a->tag,nel_a,bissa_b->tag,nel_b);
	...
	if(size_per_el_a!=size_per_el_b)
	  crash("while copying, vector %s contains %d bytes per el and vector %s contains %d",
		bissa_a->tag,size_per_el_a,bissa_b->tag,size_per_el_b);
	
	//perform the copy
	BISSA_PARALLEL_LOOP(i,0,nel_a)
	  memcpy((char*)a+i*size_per_el_a,(char*)b+i*size_per_el_a,size_per_el_a);
	
	//copy the flag
	bissa_a->flag=bissa_b->flag;
  ...
  void internal_bissa_free(char **arr,const char *file,int line)
  {
	...
	if(arr!=NULL)
	  {
	    bissa_vect *vect=(bissa_vect*)((char*)(*arr)-sizeof(bissa_vect));
	    bissa_vect *prev=vect->prev;
	    bissa_vect *next=vect->next;
  ...
  {
    GET_THREAD_ID();
    char *buf=bissa_malloc("buf",sel*nel,char);
    
    //copy in the buffer
    BISSA_PARALLEL_LOOP(sour,0,nel) memcpy(buf+order[sour]*sel,vect+sour*sel,sel);
    THREAD_BARRIER();
    BISSA_PARALLEL_LOOP(sour,0,nel) memcpy(vect+sour*sel,buf+sour*sel,sel);
