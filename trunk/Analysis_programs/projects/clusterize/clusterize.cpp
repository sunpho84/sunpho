...
typedef char sun_string[1024];

int nlevel;
sun_string *var_name;
int *var_value_tot;
int *required_header;
sun_string **var_values;
sun_string *level_header_templ;

void prepare_header(sun_string header,sun_string templ,int *level_entry)
{
  ...
  for(int ilevel=0;ilevel<nlevel;ilevel++)
      ...
      if(lfirst_part>=0 && lfirst_part<lheader)
	{
	  ...
	  memcpy(header,temp_header,sizeof(sun_string));
	}
    }
