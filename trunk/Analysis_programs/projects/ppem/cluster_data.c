...
int main(int narg,char **arg)
{
  ...
  if(ndoubles!=ndoubles_exp) crash("ndoubles do not agree with expectation (%d)",ndoubles_exp);
  ...
  for(int k2=0;k2<nk;k2++)
	...
	for(int imom1=0;imom1<nmoms;imom1++)
	  for(int ri=0;ri<2;ri++)
	    for(int t=0;t<T;t++)
	      {
		int pos_in=ri+2*(t+T*(imom1+nmoms*(imom2+nmoms*(k1+nk*k2))));
		...
		if(ri==0&&imom2==0&&imom2==0&&k1==0&&k2==0) printf("%lg\n",jacks[0]);
	      }
