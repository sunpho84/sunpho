...
int main(int narg,char **arg)
{
  ...
  read_formatted_from_file_expecting((char*)&include_a2,an_input_file,"%d","include_a2");
  ...
  for(int iens=0;iens<nens;iens++)
    {
      ...
      jvec temp(10,njack);
      jack el(njack);
      temp.load(combine("../%s/%s",path,meson_name).c_str(),0);
      el=temp[0];
      
      //write the bare data table
      bare_data_table<<path<<" "<<smart_print(el)<<endl;
      
      //load iboot
      int iboot_jack[100];
      load_iboot(iboot_jack,path);
      
      boot_from_jack(F[iens],el,iboot_jack);
    }
